/**
 * publish_historia_3b.js
 * Publicación masiva: Historia, Geografía y Ciencias Sociales — 3° Básico
 */
...
const COMMON_CONFIG = {
    ...
    basePath: 'D:\\Users\\Pablo\\Desktop\\ESCRITORIO TRABAJO 2023\\mi aula editado\\PRIMER CICLO\\ASIGNATURAS 3 bas\\historia LIS'
};

const TEMP_DIR = path.join(__dirname, 'temp_cleaned', 'historia_3b');
const MONTH_UNIT_MAP = {
    'marzo': 'Unidad 1', 'abril': 'Unidad 1', 'mayo': 'Unidad 2',
...
function generarMetadatos(item) {
    ...
    const descripcion = `
        ...
        <ul>
            ...
            <li>Objetivos de Aprendizaje (OA) de Historia y Geografía para NB2.</li>
            <li>Indicadores de evaluación.</li>
            <li>Actividades organizadas.</li>
        </ul><br>
    ...
    `;
    const tags = ['Historia', 'Geografía', 'Ciencias Sociales', 'Tercero Básico', 'Planificación', item.mes,
...
(async () => {
    console.log(`\n🗺️  HISTORIA 3° BÁSICO — Limpieza y Publicación`);
    console.log('='.repeat(60));
    ...
    try {
        ...
        for (const item of archivos) {
            ...
            } catch (e) { console.error(`   ⚠️ Timeout esperando redirección.`); await page.screenshot({ path: path.join(__dirname, `historia3b_error_${item.mes}.png`) }); }
            await page.waitForTimeout(3000);
        }
        console.log('\n🏁 FIN — Publicación Historia 3° Básico completada exitosamente.');
    } catch (err) { console.error('❌ ERROR FATAL:', err); await page.screenshot({ path: path.join(__dirname, 'historia3b_fatal.png') }); }
