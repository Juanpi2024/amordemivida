...
// Configuración Base - ORIENTACIÓN 2°
const COMMON_CONFIG = {
    precio: '6',
    minAge: '6',
    maxAge: '8',
    ...
    basePath: 'D:\\Users\\Pablo\\Desktop\\ESCRITORIO TRABAJO 2023\\mi aula editado\\PRIMER CICLO\\segundo basico\\ASIGNATURAS\\orientacion'
};

// Mapeo de meses (Ajustado a estructura Semestral/Anual de Orientación)
const MONTH_UNIT_MAP = {
    'marzo': 'Unidad 1', 'abril': 'Unidad 1', 'mayo': 'Unidad 1', 'junio': 'Unidad 1',
    'julio': 'Unidad 2', 'agosto': 'Unidad 2', 'septiembre': 'Unidad 2',
    'octubre': 'Unidad 2', 'noviembre': 'Unidad 2', 'diciembre': 'Unidad 2'
};
    ...
    for (const file of files) {
        ...
        if (stat.isDirectory()) {
            ...
            if (pattern.test(file) && /\.(doc|docx)$/i.test(file)) {
                ...
                // Determinar Unidad basado en carpeta padre si es posible, sino mapa
                let unidad = 'General';
                if (mesEncontrado) {
                    unidad = MONTH_UNIT_MAP[mesEncontrado.toLowerCase()];
                    // Override si está explícitamente en una carpeta "unidad X"
                    if (lowerPath.includes('unidad 1')) unidad = 'Unidad 1';
                    if (lowerPath.includes('unidad 2')) unidad = 'Unidad 2';
                    if (lowerPath.includes('unidad 3')) unidad = 'Unidad 3'; // Por si acaso
                    if (lowerPath.includes('unidad 4')) unidad = 'Unidad 4'; // Por si acaso
...
function generarMetadatos(item) {
    ...
    const descripcion = `
        ...
        <ul>
            ...
            <li>Participación y Pertenencia.</li>
            <li>Trabajo Escolar.</li>
        </ul><br>
        ...
        <ul>
            ...
            <li>Fomenta el desarrollo socioemocional.</li>
        </ul><br>
...
(async () => {
    console.log(`🏰 LENIN ORIENTACION 2D: Iniciando proceso de Limpieza y Publicación...`);
    ...
    cleanFiles.forEach(f => console.log(`   - ${f.mes} (${f.unidad}): ${f.name}`));
    ...
    try {
        ...
        for (const item of cleanFiles) {
            ...
            try {
                ...
                await page.screenshot({ path: `orientacion_error_${item.mes}.png` });
            }

            await page.waitForTimeout(3000);
        }

        console.log('\n🏁 FIN DEL PROCESO DE ORIENTACIÓN.');
