/**
 * publish_musica_1b.js
 * Publicación masiva: Música — 1° Básico
 */
...
const COMMON_CONFIG = {
    ...
    asignatura: 'Música',
    nivel: 'Primer Básico',
    basePath: 'D:\\Users\\Pablo\\Desktop\\ESCRITORIO TRABAJO 2023\\mi aula editado\\PRIMER CICLO\\primero basico asig\\musica LIS'
};

const TEMP_DIR = path.join(__dirname, 'temp_cleaned', 'musica_1b');
...
function generarMetadatos(item) {
    ...
    const descripcion = `
        ...
        <ul>
            ...
            <li>Objetivos de Aprendizaje (OA) de Música.</li>
            <li>Actividades de escucha activa, expresión corporal y canto.</li>
            <li>Exploración de instrumentos rítmicos y sonidos del entorno.</li>
            <li>Indicadores de evaluación y recursos auditivos sugeridos.</li>
        </ul><br>
        <strong>💡 ¿Por qué descargar este recurso?</strong>
        <ul>
            <li>Ahorra tiempo en la preparación de clases de artes musicales.</li>
            <li>Alineado al currículo del Ministerio de Educación.</li>
            <li>Ideal para despertar la sensibilidad artística y el gusto por la música en 1° Básico.</li>
        </ul><br>
    ...
    const tags = [COMMON_CONFIG.asignatura, COMMON_CONFIG.nivel, 'Planificación', item.mes,
        'Música', 'Artes Musicales', 'Canto', 'NB1', item.unidad,
        'Clase a clase', 'Material Docente', 'Primer Básico', 'Expresión corporal'];
...
(async () => {
    console.log(`\n🎶  MÚSICA 1° BÁSICO — Limpieza y Publicación`);
    console.log('='.repeat(60));
    ...
    try {
        ...
        for (const item of archivos) {
            ...
            } catch (e) { console.error(`   ⚠️ Timeout.`); await page.screenshot({ path: path.join(__dirname, `musica1b_error_${item.mes}.png`) }); }
            await page.waitForTimeout(3000);
        }
        console.log('\n🏁 FIN — Música 1° Básico completado.');
    } catch (err) { console.error('❌ ERROR:', err); await page.screenshot({ path: path.join(__dirname, 'musica1b_fatal.png') }); }
