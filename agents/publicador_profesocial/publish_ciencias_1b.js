/**
 * publish_ciencias_1b.js
 * Publicación masiva: Ciencias Naturales — 1° Básico
 */
...
const COMMON_CONFIG = {
    ...
    nivel: 'Primer Básico',
    basePath: 'D:\\Users\\Pablo\\Desktop\\ESCRITORIO TRABAJO 2023\\mi aula editado\\PRIMER CICLO\\primero basico asig\\cs naturales LIS'
};

const TEMP_DIR = path.join(__dirname, 'temp_cleaned', 'ciencias_1b');
...
function generarMetadatos(item) {
    ...
    const descripcion = `
        ...
        <ul>
            ...
            <li>Objetivos de Aprendizaje (OA) de Ciencias Naturales.</li>
            <li>Enfoque en la observación, exploración y cuidado de la naturaleza.</li>
            <li>Actividades experimentales sencillas e indagación.</li>
            <li>Fomento de actitudes de curiosidad y responsabilidad ambiental.</li>
        </ul><br>
        <strong>💡 ¿Por qué descargar este recurso?</strong>
        <ul>
            <li>Ahorra tiempo valioso de planificación mensual.</li>
            <li>Alineado al currículo vigente del Ministerio de Educación.</li>
            <li>Ideal para fomentar el pensamiento científico desde edades tempranas.</li>
        </ul><br>
    ...
    const tags = [COMMON_CONFIG.asignatura, COMMON_CONFIG.nivel, 'Planificación', item.mes,
        'Ciencias Naturales', 'Naturaleza', 'Ciencias', 'NB1', item.unidad,
        'Clase a clase', 'Material Docente', 'Primer Básico', 'Indagación'];
...
(async () => {
    console.log(`\n🔬  CIENCIAS NATURALES 1° BÁSICO — Limpieza y Publicación`);
    console.log('='.repeat(60));
    ...
    try {
        ...
        for (const item of archivos) {
            ...
            } catch (e) { console.error(`   ⚠️ Timeout.`); await page.screenshot({ path: path.join(__dirname, `ciencias1b_error_${item.mes}.png`) }); }
            await page.waitForTimeout(3000);
        }
        console.log('\n🏁 FIN — Ciencias Naturales 1° Básico completado.');
    } catch (err) { console.error('❌ ERROR:', err); await page.screenshot({ path: path.join(__dirname, 'ciencias1b_fatal.png') }); }
