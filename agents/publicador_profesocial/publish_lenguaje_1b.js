/**
 * publish_lenguaje_1b.js
 * Publicación masiva: Lenguaje y Comunicación — 1° Básico
 */
...
const COMMON_CONFIG = {
    ...
    nivel: 'Primer Básico',
    basePath: 'D:\\Users\\Pablo\\Desktop\\ESCRITORIO TRABAJO 2023\\mi aula editado\\PRIMER CICLO\\primero basico asig\\lenguaje LIS'
};

const TEMP_DIR = path.join(__dirname, 'temp_cleaned', 'lenguaje_1b');
...
function generarMetadatos(item) {
    ...
    const descripcion = `
        ...
        <ul>
            ...
            <li>Objetivos de Aprendizaje (OA) de Lenguaje y Comunicación.</li>
            <li>Actividades de lectura inicial, escritura y comprensión lectora.</li>
            <li>Indicadores de evaluación y estrategias diferenciadas.</li>
            <li>Recursos literarios y textos sugeridos.</li>
        </ul><br>
        ...
        <ul>
            ...
            <li>Ideal para docentes de 1° Básico en proceso de lectoescritura inicial.</li>
        </ul><br>
    ...
    const tags = [COMMON_CONFIG.asignatura, COMMON_CONFIG.nivel, 'Planificación', item.mes,
        'Lenguaje', 'Comunicación', 'Lectoescritura', 'NB1', item.unidad,
        'Clase a clase', 'Material Docente', 'Primer Básico', 'Lectura inicial'];
...
(async () => {
    console.log(`\n📚  LENGUAJE 1° BÁSICO — Limpieza y Publicación`);
    console.log('='.repeat(60));
    ...
    try {
        ...
        for (const item of archivos) {
            ...
            } catch (e) { console.error(`   ⚠️ Timeout.`); await page.screenshot({ path: path.join(__dirname, `lenguaje1b_error_${item.mes}.png`) }); }
            await page.waitForTimeout(3000);
        }
        console.log('\n🏁 FIN — Lenguaje 1° Básico completado.');
    } catch (err) { console.error('❌ ERROR:', err); await page.screenshot({ path: path.join(__dirname, 'lenguaje1b_fatal.png') }); }
