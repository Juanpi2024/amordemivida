/**
 * publish_historia_1b.js
 * Publicación masiva: Historia, Geografía y Ciencias Sociales — 1° Básico
 */
...
const COMMON_CONFIG = {
    ...
    nivel: 'Primer Básico',
    basePath: 'D:\\Users\\Pablo\\Desktop\\ESCRITORIO TRABAJO 2023\\mi aula editado\\PRIMER CICLO\\primero basico asig\\historia LIS'
};

const TEMP_DIR = path.join(__dirname, 'temp_cleaned', 'historia_1b');
...
function generarMetadatos(item) {
    ...
    const descripcion = `
        ...
        <ul>
            ...
            <li>Objetivos de Aprendizaje (OA) de Historia y Geografía.</li>
            <li>Indicadores de evaluación y habilidades.</li>
            <li>Actividades de identidad, territorio y convivencia.</li>
            <li>Recursos y materiales sugeridos.</li>
        </ul><br>
        ...
        <ul>
            ...
            <li>Ideal para docentes de 1° Básico (NB1).</li>
        </ul><br>
    ...
    const tags = [COMMON_CONFIG.asignatura, COMMON_CONFIG.nivel, 'Planificación', item.mes,
        'Historia', 'Geografía', 'Ciencias Sociales', 'NB1', item.unidad,
        'Clase a clase', 'Material Docente', 'Primer Básico'];
...
(async () => {
    console.log(`\n🗺️  HISTORIA 1° BÁSICO — Limpieza y Publicación`);
    console.log('='.repeat(60));
    ...
    try {
        ...
        for (const item of archivos) {
            ...
            } catch (e) { console.error(`   ⚠️ Timeout.`); await page.screenshot({ path: path.join(__dirname, `historia1b_error_${item.mes}.png`) }); }
            await page.waitForTimeout(3000);
        }
        console.log('\n🏁 FIN — Historia 1° Básico completado.');
    } catch (err) { console.error('❌ ERROR:', err); await page.screenshot({ path: path.join(__dirname, 'historia1b_fatal.png') }); }
