...
const COMMON_CONFIG = {
    ...
    basePath: 'D:\\Users\\Pablo\\Desktop\\ESCRITORIO TRABAJO 2023\\mi aula editado\\PRIMER CICLO\\segundo basico\\ASIGNATURAS\\Historia'
};
        ...
        if (stat.isDirectory()) {
            ...
            if (pattern.test(file) && /\.(doc|docx)$/i.test(file)) {
                ...
                if (mesEncontrado) {
                    ...
                    // console.log(`⚠️ Archivo ignorado (sin mes): ${file}`);
                }
            }
        }
    }
...
function generarMetadatos(item) {
    ...
    const descripcion = `
        ...
        <strong>✅ Contenidos y Habilidades:</strong>
        <ul>
            <li>Planificación diaria estructurada (Inicio, Desarrollo, Cierre).</li>
            <li>Objetivos de Aprendizaje (OA) cubiertos en el mes.</li>
            <li>Indicadores de evaluación claros.</li>
            <li>Actividades para el desarrollo del pensamiento histórico y geográfico.</li>
        </ul><br>
        
        <strong>💡 Beneficios:</strong>
        <ul>
            <li>Material listo para editar y presentar.</li>
            <li>Alineado al currículum nacional.</li>
...
(async () => {
    console.log(`🏰 LENIN HISTORY 2D: Iniciando proceso de Limpieza y Publicación...`);
    ...
    try {
        ...
        for (const item of cleanFiles) {
            ...
            // Si la limpieza base funcionó (o copió el archivo), seguimos trabajando sobre 'cleanedPath'
            if (metaResult.success || (metaResult.outputPath && fs.existsSync(metaResult.outputPath))) {
                let currentPath = cleanedPath;

                // PASO 2: ELIMINAR ENCABEZADOS Y PIES DE PÁGINA (Para quitar logos/fondos "Mi Aula")
                console.log(`   💣 Eliminando Headers y Footers (adiós marca de agua)...`);
                const headerResult = await removeHeaders(currentPath, currentPath);
                if (headerResult.success && headerResult.modified) {
                    console.log(`      ✅ Headers/Footers eliminados exitosamente.`);
                } else if (headerResult.success) {
                    console.log(`      ℹ️ No se detectaron headers para eliminar.`);
                } else {
                    console.warn(`      ⚠️ Error eliminando headers: ${headerResult.error}`);
                }

                // PASO 3: Reemplazo de texto (por si queda algo en el cuerpo)
                ...
                if (replaceResult.success) {
                    ...
                    console.log(`   ✅ Texto del cuerpo revisado/reemplazado.`);
                    if (replaceResult.matches > 0) console.log(`      (Se hicieron ${replaceResult.matches} reemplazos de texto)`);
                } else {
                    console.error(`   ❌ Falló reemplazo de texto: ${replaceResult.error}`);
                    // Aún así usamos el archivo que ya tiene metadatos y headers limpios
                    finalUploadPath = currentPath;
                }
            } else {
            ...
            try {
                ...
                await page.screenshot({ path: `historia_error_${item.mes}.png` });
            }

            await page.waitForTimeout(3000);
        }

        console.log('\n🏁 FIN DEL PROCESO DE HISTORIA.');
