/**
 * publish_ingles_1b.js
 * Publicación masiva: Inglés — 1° Básico
 */
...
const COMMON_CONFIG = {
    ...
    asignatura: 'Inglés',
    nivel: 'Primer Básico',
    basePath: 'D:\\Users\\Pablo\\Desktop\\ESCRITORIO TRABAJO 2023\\mi aula editado\\PRIMER CICLO\\primero basico asig\\ingles LIS'
};

const TEMP_DIR = path.join(__dirname, 'temp_cleaned', 'ingles_1b');
...
function generarMetadatos(item) {
    const titulo = `Planificación Clase a Clase (Inglés): ${item.mes} | ${COMMON_CONFIG.asignatura} ${COMMON_CONFIG.nivel}`;
    const descripcion = `
        <strong>🇬🇧 Planificación Detallada Clase a Clase (Inglés) - Mes de ${item.mes}</strong><br><br>
        ...
        <ul>
            <li>Planificación diaria estructurada (Warm-up, Presentation, Practice, Production).</li>
            <li>Objetivos de Aprendizaje (OA) de Inglés.</li>
            <li>Vocabulario clave y estructuras gramaticales básicas.</li>
            <li>Actividades lúdicas, canciones y juegos para primer grado.</li>
            <li>Indicadores de evaluación y sugerencias de recursos.</li>
        </ul><br>
        ...
        <ul>
            ...
            <li>Diseñado para una iniciación amigable y efectiva al idioma inglés.</li>
        </ul><br>
    ...
    const tags = [COMMON_CONFIG.asignatura, COMMON_CONFIG.nivel, 'Planificación', item.mes,
        'Inglés', 'English', 'EFL', 'NB1', item.unidad,
        'Clase a clase', 'Material Docente', 'Primer Básico', 'Vocabulario'];
...
(async () => {
    console.log(`\n🇬🇧  INGLÉS 1° BÁSICO — Limpieza y Publicación`);
    console.log('='.repeat(60));
    ...
    try {
        ...
        for (const item of archivos) {
            ...
            } catch (e) { console.error(`   ⚠️ Timeout.`); await page.screenshot({ path: path.join(__dirname, `ingles1b_error_${item.mes}.png`) }); }
            await page.waitForTimeout(3000);
        }
        console.log('\n🏁 FIN — Inglés 1° Básico completado.');
    } catch (err) { console.error('❌ ERROR:', err); await page.screenshot({ path: path.join(__dirname, 'ingles1b_fatal.png') }); }
