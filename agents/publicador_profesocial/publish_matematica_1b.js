/**
 * publish_matematica_1b.js
 * Publicación masiva: Matemática — 1° Básico
 */
...
const COMMON_CONFIG = {
    ...
    nivel: 'Primer Básico',
    basePath: 'D:\\Users\\Pablo\\Desktop\\ESCRITORIO TRABAJO 2023\\mi aula editado\\PRIMER CICLO\\primero basico asig\\matematica LIS'
};

const TEMP_DIR = path.join(__dirname, 'temp_cleaned', 'matematica_1b');
...
function generarMetadatos(item) {
    ...
    const descripcion = `
        ...
        <ul>
            ...
            <li>Objetivos de Aprendizaje (OA) de Matemática (Números, Geometría, Medición).</li>
            <li>Actividades prácticas y ejercicios de razonamiento lógico.</li>
            <li>Indicadores de evaluación y sugerencias metodológicas.</li>
            <li>Uso de material concreto y representaciones pictóricas.</li>
        </ul><br>
        ...
        <ul>
            ...
            <li>Ideal para docentes de 1° Básico que buscan un enfoque dinámico en matemáticas.</li>
        </ul><br>
    ...
    const tags = [COMMON_CONFIG.asignatura, COMMON_CONFIG.nivel, 'Planificación', item.mes,
        'Matemática', 'Números', 'Geometría', 'NB1', item.unidad,
        'Clase a clase', 'Material Docente', 'Primer Básico', 'Razonamiento lógico'];
...
(async () => {
    console.log(`\n🔢  MATEMÁTICA 1° BÁSICO — Limpieza y Publicación`);
    console.log('='.repeat(60));
    ...
    try {
        ...
        for (const item of archivos) {
            ...
            } catch (e) { console.error(`   ⚠️ Timeout.`); await page.screenshot({ path: path.join(__dirname, `matematica1b_error_${item.mes}.png`) }); }
            await page.waitForTimeout(3000);
        }
        console.log('\n🏁 FIN — Matemática 1° Básico completado.');
    } catch (err) { console.error('❌ ERROR:', err); await page.screenshot({ path: path.join(__dirname, 'matematica1b_fatal.png') }); }
