/**
 * publish_visuales_1b.js
 * Publicación masiva: Artes Visuales — 1° Básico
 */
...
const COMMON_CONFIG = {
    ...
    nivel: 'Primer Básico',
    basePath: 'D:\\Users\\Pablo\\Desktop\\ESCRITORIO TRABAJO 2023\\mi aula editado\\PRIMER CICLO\\primero basico asig\\visuales LIS'
};

const TEMP_DIR = path.join(__dirname, 'temp_cleaned', 'visuales_1b');
...
function generarMetadatos(item) {
    ...
    const descripcion = `
        ...
        <ul>
            <li>Planificación diaria estructurada (Explicación, Experimentación, Cierre).</li>
            <li>Objetivos de Aprendizaje (OA) de Artes Visuales.</li>
            <li>Enfoque en la expresión personal y el uso de diversos materiales artísticos.</li>
            <li>Actividades de dibujo, pintura, modelado y apreciación del arte.</li>
            <li>Sugerencias de materiales y guía para la evaluación de procesos creativos.</li>
        </ul><br>
        <strong>💡 ¿Por qué descargar este recurso?</strong>
        <ul>
            <li>Ahorra tiempo en la planificación de tus clases de artes plásticas.</li>
            <li>Alineado al currículo del Ministerio de Educación de Chile.</li>
            <li>Ideal para potenciar la creatividad y la motricidad fina en alumnos de 1° Básico.</li>
        </ul><br>
    ...
    const tags = [COMMON_CONFIG.asignatura, COMMON_CONFIG.nivel, 'Planificación', item.mes,
        'Artes Visuales', 'Arte', 'Creatividad', 'NB1', item.unidad,
        'Clase a clase', 'Material Docente', 'Primer Básico', 'Expresión Artística'];
...
(async () => {
    console.log(`\n🎨  ARTES VISUALES 1° BÁSICO — Limpieza y Publicación`);
    console.log('='.repeat(60));
    ...
    try {
        ...
        for (const item of archivos) {
            ...
            } catch (e) { console.error(`   ⚠️ Timeout.`); await page.screenshot({ path: path.join(__dirname, `visuales1b_error_${item.mes}.png`) }); }
            await page.waitForTimeout(3000);
        }
        console.log('\n🏁 FIN — Artes Visuales 1° Básico completado.');
    } catch (err) { console.error('❌ ERROR:', err); await page.screenshot({ path: path.join(__dirname, 'visuales1b_fatal.png') }); }
