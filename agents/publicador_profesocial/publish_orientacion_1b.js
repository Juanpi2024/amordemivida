/**
 * publish_orientacion_1b.js
 * Publicación masiva: Orientación — 1° Básico
 */
...
const COMMON_CONFIG = {
    ...
    nivel: 'Primer Básico',
    basePath: 'D:\\Users\\Pablo\\Desktop\\ESCRITORIO TRABAJO 2023\\mi aula editado\\PRIMER CICLO\\primero basico asig\\orientacion LIS'
};

const TEMP_DIR = path.join(__dirname, 'temp_cleaned', 'orientacion_1b');
...
function generarMetadatos(item) {
    ...
    const descripcion = `
        ...
        <ul>
            ...
            <li>Objetivos de Aprendizaje (OA) de Orientación.</li>
            <li>Enfoque en el crecimiento personal, relaciones interpersonales y participación.</li>
            <li>Actividades de reflexión, diálogo y formación de valores.</li>
            <li>Guía para el docente sobre convivencia escolar y autoconocimiento.</li>
        </ul><br>
        <strong>💡 ¿Por qué descargar este recurso?</strong>
        <ul>
            <li>Ahorra tiempo en la planificación de tus horas de orientación.</li>
            <li>Alineado al currículo nacional vigente.</li>
            <li>Herramienta fundamental para el acompañamiento socioemocional de alumnos de 1° Básico.</li>
        </ul><br>
    ...
    const tags = [COMMON_CONFIG.asignatura, COMMON_CONFIG.nivel, 'Planificación', item.mes,
        'Orientación', 'Formación de Valores', 'Socioemocional', 'NB1', item.unidad,
        'Clase a clase', 'Material Docente', 'Primer Básico', 'Convivencia'];
...
(async () => {
    console.log(`\n🌱  ORIENTACIÓN 1° BÁSICO — Limpieza y Publicación`);
    console.log('='.repeat(60));
    ...
    try {
        ...
        for (const item of archivos) {
            ...
            } catch (e) { console.error(`   ⚠️ Timeout.`); await page.screenshot({ path: path.join(__dirname, `orientacion1b_error_${item.mes}.png`) }); }
            await page.waitForTimeout(3000);
        }
        console.log('\n🏁 FIN — Orientación 1° Básico completado.');
    } catch (err) { console.error('❌ ERROR:', err); await page.screenshot({ path: path.join(__dirname, 'orientacion1b_fatal.png') }); }
